...
> Answer: Client side routing allows less refreshes on a page which makes it a great UI experience overall.  Client side routing also is faster to use because when the page doesn't refresh on every click or command from the user, the page looks like it is happening instantly when in realtime the data was already rendered once the page initially rendered.
